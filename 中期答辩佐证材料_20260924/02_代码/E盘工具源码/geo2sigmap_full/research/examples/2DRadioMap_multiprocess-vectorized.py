...
def process_batch(batch_indices, cell_size,faces_name, verts_name, values_name, x_coords_name, y_coords_name,
                         faces_shape, verts_shape, values_shape,x_coords_shape, y_coords_shape ):
    # global shm_faces, shm_verts, shm_values, shm_x_coords, shm_y_coords
    # global g_faces_shape, g_verts_shape, g_values_shape, g_x_coords_shape, g_y_coords_shape
    ...
    shm_x_coords = shared_memory.SharedMemory(name=x_coords_name)
    shm_y_coords = shared_memory.SharedMemory(name=y_coords_name)
    faces = np.ndarray(faces_shape, dtype=np.int64, buffer=shm_faces.buf)
    vertices = np.ndarray(verts_shape, dtype=np.float64, buffer=shm_verts.buf)
    triangle_values = np.ndarray(values_shape, dtype=np.float32, buffer=shm_values.buf)
    x_coords_data = np.ndarray(x_coords_shape, dtype=np.float64, buffer=shm_x_coords.buf)
    y_coords_data = np.ndarray(y_coords_shape, dtype=np.float64, buffer=shm_y_coords.buf)
        ...
        for i in batch_indices:
            ...
            for xi in range(x_start, x_end):
                for yi in range(y_start, y_end):
                    ...
                    #     x_coords[xi] + cell_size, y_coords[yi] + cell_size
                    # )
                    min_x.append(x_coords[xi])
                    min_y.append(y_coords[yi])
                    max_x.append(x_coords[xi]+ cell_size)
                    max_y.append(y_coords[yi]+ cell_size)
                    #potencial_cells.append(cell)
                    potencial_cells_indices.append((xi, yi))
            potencial_cells = shapely.box(min_x, min_y, max_x, max_y)
            #print("len(potencial_cells): ", len(potencial_cells))
...
def main(ply_file, color_file, cell_size, n_jobs, batch_size, output_file):
    ...
    #shm_cell_boxes = shared_memory.SharedMemory(create=True, size=cell_boxes.nbytes)
    ...
    with ProcessPoolExecutor(
        ...
        futures = [executor.submit(process_batch, batch, cell_size,shm_faces.name, shm_verts.name, shm_vals.name,shm_x_coords.name, shm_y_coords.name, 
                  face_shape, vert_shape, vals_shape, x_coords_shape, y_coords_shape)
                   for batch in batches]
    ...
    shm_x_coords.close();shm_x_coords.unlink()
    shm_y_coords.close();shm_y_coords.unlink()
