# 仿真与固定基站 Stage 2 历史过程记录

> 本文保留早期 WinProp 方案、Sionna 切换过程和运行命令。它不是当前项目入口；当前阶段状态与 V5 请看 [README](README.md) 和 [当前路线](SIONNA_CURRENT.md)。历史命令依赖原机路径和软件，执行前需另设输出目录，避免覆盖已完成实验。
> 核对说明：3,200 瓦片 Sionna 计划实际完成 3 个；27,360 v3 已完成全量；固定基站 Stage 2B 配置上限 200 轮，实际在 epoch 154 停止并完成测试。下文 production/formal 描述当时的方案，不代表这些历史轮次全部完成。
...
& $python D:\桌面\dac\projects\dpm_unet_stage1\batch_infer.py `
...
The paper-frequency result above remains immutable. The historical 3.5 GHz Sionna
...
The historical 3,200-tile plan (only 3 tiles completed; not the current training dataset) used
