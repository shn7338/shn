...
def finalize_dataset(
    ...
    normalization = {
        "version": int(config["version"]),
        "source": f"completed training split of {config['name']}",
    ...
    readme = render_dataset_readme(config, entries, normalization)
...
def render_dataset_readme(
    ...
    """Describe a finalized dataset from its actual manifest, not Pilot constants."""
    splits = ("train", "val", "test")
    buildings = [len({e["tile"] for e in entries if e["split"] == s}) for s in splits]
    sites = [sum(e["split"] == s for e in entries) for s in splits]
    directions = [sum(len(e["azimuths_deg"]) for e in entries if e["split"] == s) for s in splits]
    sampling = config["location_sampling"]
    if sampling.get("strategy") == "radial_bins":
        location_description = f"按距瓦片中心的径向区间均衡采样：{sampling['center_radius_bins_m']} m"
    elif sampling.get("one_site_per_quadrant"):
        location_description = "每个建筑场景四象限各采样 1 个站点"
    else:
        location_description = f"位置采样规则见配置 location_sampling：{sampling}"
    sparse = config["sparse_measurements"]
    reference = config["normalization_reference"]["signal_strength"]
    config_name = Path(str(config.get("output_root", "dataset"))).name
    return f"""# 随机基站反演数据：{config_name}

数据配置：`{config['name']}`；生成器：Sionna RT {config['runtime_sionna_version']}。
状态以 `FINAL_REPORT.json` 的 `complete_and_verified` 和本机 `VALIDATION_REPORT.json` 为准。

- 建筑场景（瓦片）：{sum(buildings)}（train/val/test = {'/'.join(map(str, buildings))}）；这里统计的是场景数，不是单体建筑栋数，沿用原空间块划分。
- 基站站点：{sum(sites)}（train/val/test = {'/'.join(map(str, sites))}）；{location_description}。
- 传播图：{sum(sites) + sum(directions)}（每站点 1 张各向同性图，另有各方向图）。
- 方向训练样本：{sum(directions)}（train/val/test = {'/'.join(map(str, directions))}）。
- 稀疏点：每张方向图 {sparse['point_count']} 个有效室外点；噪声标准差 {sparse['noise_std_db']} dB，异常点概率 {sparse['outlier_probability']}。
...
- `sparse_flat_indices`, `sparse_signal_strength_db`：`[4,{sparse['point_count']}]` 稀疏测点；在线用 `row=index//128, col=index%128` 还原掩码。
...
`sparse_signal_strength_norm_reference` 使用已有 Stage 2B 的参考归一化（mean={reference['mean_db']} dB，std={reference['std_db']} dB）。**实际模型应使用训练时的统计**：V5 训练脚本通过 `irt_normalization` 读取固定基站 Sionna v3 的统计，并显式覆盖数据集默认值；估计器输入使用其 checkpoint 元数据中的统计。不能把上述数据集默认公式无条件用于所有模型。
