...
$OutputDir = Join-Path $DesktopDir "dac\models\stage2b_latent_fusion_scale4_v4"
$HistoryPath = Join-Path $OutputDir "history.csv"
$SummaryPath = Join-Path $OutputDir "training_summary.json"
$Host.UI.RawUI.WindowTitle = "Stage2B V4 latent-fusion training monitor"

while ($true) {
    Clear-Host
    Write-Host "Stage2B V4 latent-fusion training monitor" -ForegroundColor Cyan
    ...
    if (Test-Path -LiteralPath $HistoryPath) {
        ...
        if ($Rows.Count -gt 0) {
            ...
            Write-Host ("Epoch: {0}/12    LR: {1:E3}" -f `
