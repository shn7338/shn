...
def migrated_e_location(name: str) -> Path:
    """Current location of a directory originally under E:/ ."""
    if name == "dac_bs_inversion_scale4_v1_work":
        group = "02_inversion/E_work"
    elif name == "dac_checkpoints":
        group = "07_backup"
    elif name == "dac_sionna_rt_env":
        group = "08_runtime"
    elif name == "dac_tools":
        group = "05_legacy_code"
    elif name.startswith("dac_irt_"):
        group = "04_simulation/irt"
    elif name.startswith("dac_sionna_"):
        group = "04_simulation/sionna"
    elif name.startswith("dac_winprop_"):
        group = "04_simulation/winprop"
    elif name.startswith("dac_stage"):
        group = "03_runs/E_stage"
    else:
        raise ValueError(name)
    return REPO / group / name
...
def d_records() -> None:
    ...
    for name in ["metadata.json", "building_height_4m.npy", "path_gain_4m.npy"]:
        copy(d / "01_data/prepared_4m/tile_000001" / name, win / "预处理样例" / name, "预处理样本")
    for name in ["normalization_summary.csv", "normalization.json", "splits.csv", "tx_feature_summary.csv", "train_tiles.txt", "val_tiles.txt", "test_tiles.txt"]:
        copy(d / "01_data/normalized_4m_v1" / name, win / "标准化与划分" / name, "预处理记录")
    for name in ["bs_inversion_pilot_v1", "bs_inversion_scale4_v1"]:
        src = d / "02_inversion" / name
    ...
    sample = d / "02_inversion/bs_inversion_scale4_v1/samples/test"
    for suffix in ["json", "npz"]:
        copy(sample / f"tile_000011_site00.{suffix}", Path("01_数据记录/04_随机基站数据/样本") / f"tile_000011_site00.{suffix}", "随机基站测试样本")
    model_dirs = [
        "03_runs/stage1_dpm_unet_runs/baseline_v1",
        "03_runs/models/stage2a_iso_refine_sionna35_depth8_paper_v1",
        "03_runs/models/stage2b_directional_ss_sionna35_depth8_paper_v1",
        "03_runs/models/bs_parameter_estimator_scale4_v2",
        "03_runs/models/stage2b_position_encoding_scale4_v3",
        "03_runs/models/stage2b_latent_fusion_scale4_v4",
        "03_runs/models/stage2b_multiscale_adapter_scale4_v5",
        "03_runs/models/joint_estimator_stage2b_scale4_v6",
    ]
    ...
    copy(d / "03_runs/stage1_results.md", Path("01_数据记录/05_训练评估记录/stage1_results.md"), "第一阶段报告")
    experiments = d / "03_runs/experiments"
