# 随机基站反演数据：bs_inversion_scale4_v1

数据配置：`bs_inversion_random_location_scale4_v1`；生成器：Sionna RT 2.0.1。
状态以 `FINAL_REPORT.json` 的 `complete_and_verified` 和本机 `VALIDATION_REPORT.json` 为准。

- 建筑场景（瓦片）：1536（train/val/test = 1024/256/256）；这里统计的是场景数，不是单体建筑栋数，沿用原空间块划分。
- 基站站点：6144（train/val/test = 4096/1024/1024）；按距瓦片中心的径向区间均衡采样：[[0.0, 64.0], [64.0, 128.0], [128.0, 192.0], [192.0, 271.0]] m。
- 传播图：30720（每站点 1 张各向同性图，另有各方向图）。
- 方向训练样本：24576（train/val/test = 16384/4096/4096）。
...
`signal_norm = (signal_db - (-60.603418467186)) / 20.639905472982`
